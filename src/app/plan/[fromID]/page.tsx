...
export async function generateStaticParams(){
    ...
    return communityColleges.map(v=>({
        fromID: v[1].name.toLowerCase().replaceAll(" ", "-").replaceAll(",","%2C").toString()
    }))
}
...
export default async function PlanPage({ params }: { params: { fromID: string } }) {
    ...
    const home = communityColleges.find(v=>v[1].name.toLowerCase().replaceAll(" ", "-").replaceAll(",","%2C").toString())!
    return (
